<script lang="ts">
    ...
    const coverQualitySteps = [0.9, 0.85, 0.75, 0.65];
    const coverTargetBytes = 1 * 1024 * 1024;
    ...
    const compressCoverFile = async (file: File) => {
        ...
        try {
            ...
            const scale = Math.min(
                1,
                coverMaxDimension / Math.max(bitmap.width, bitmap.height),
            );
            ...
            const outputName = generateSafeFilename("webp");
            ...
            for (const quality of coverQualitySteps) {
                ...
                if (!blob) continue;
                const candidate = new File([blob], outputName, {
                    type: outputType,
                });
                bestFile = candidate;
                if (candidate.size <= coverTargetBytes) return candidate;
            }
                ...
                <div class="input-group tag-group">
                    <div class="tag-list">
                        {#each tags as tag}
                            <button
                                class="tag-chip"
                                on:click={() => removeTag(tag)}>#{tag} ×</button
                            >
                        {/each}
                        <input
                            class="tag-input"
